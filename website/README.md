...
├── qna.html                    ← Q&A
├── download.html               ← 다운로드
├── privacy.html                ← 개인정보처리방침 (docs/PRIVACY.md 에서 생성)
├── terms.html                  ← 이용약관 (docs/TERMS.md 에서 생성)
├── 404.html                    ← 404 페이지 (호스팅이 자동으로 사용)
├── sitemap.xml                 ← 색인 대상 9개 URL
├── robots.txt
├── scripts/
│   ├── sync-release.mjs        ← 릴리즈 값 주입 (아래 참고)
│   ├── build-updates.mjs       ← 릴리즈 본문으로 변경 이력 생성 (아래 참고)
│   ├── build-legal.mjs         ← docs/*.md 로 방침·약관 생성 (아래 참고)
│   └── md-inline.mjs           ← 두 빌더가 함께 쓰는 인라인 마크다운 변환기
└── assets/
...
**변경 이력은 별도 스크립트가 채운다.** 아래 "변경 이력" 절 참고.

손으로 채워야 하는 것은 **공지사항**(`assets/notices.js`) 하나뿐이다.
...
이 README 의 위 두 줄과 `scripts/build-updates.mjs` 의 치환 코드는 문자 자체를
가리키는 것이라 걸려도 정상이다. 그 밖에 걸리는 곳이 있으면 고칠 것.
...
사이는 **GitHub 릴리즈 본문에서 생성**된다. 손으로 고치지 말 것.

```bash
node website/scripts/build-updates.mjs
```

Pages 워크플로가 배포 때마다 돌리므로 보통은 직접 실행할 일이 없다.
`--check` 는 파일을 쓰지 않고 어긋났는지만 알려준다.

### 릴리즈 노트 쓰는 법

릴리즈 본문에서 아래 제목이 붙은 절만 사이트로 옮긴다. 나머지 절(다운로드 표,
해시 확인 방법 등)은 이미 사이트의 다른 페이지에 있으므로 무시한다.
...
목록은 `-` 로 시작하는 줄만 항목이 된다. 아는 절이 하나도 없으면 도입 문단과
원문 링크만 남기고 경고를 띄운다.
...
- **em dash 를 쓰지 말 것.** 사이트 규칙상 허용하지 않아서, 생성기가 콜론이나
  하이픈으로 바꾸고 경고를 남긴다. 처음부터 콜론으로 쓰면 문구가 그대로 간다.
- **"위 해시", "아래 표" 같은 문맥 참조를 옮겨지는 절에 넣지 말 것.** 릴리즈
  페이지에서는 바로 위에 해당 내용이 있지만, 사이트 변경 이력에는 그 절이
  오지 않아 가리키는 대상이 사라진다.
- 사이트에 넣고 싶은 내용은 반드시 위 표의 절 안에 둘 것. 다른 절에 적으면
  사이트에는 나오지 않는다.

## 법적 고지 문서

**원본은 `docs/PRIVACY.md` 와 `docs/TERMS.md` 하나뿐이다.**
`privacy.html` 과 `terms.html` 은 생성물이므로 손으로 고치지 말 것. 고쳐도 다음
빌드에서 원본 기준으로 덮어써진다(실제로 그렇게 동작하는지 시험해 두었다).

```bash
node website/scripts/build-legal.mjs
```

`--check` 를 붙이면 파일을 쓰지 않고 원본과 어긋났는지만 알려주며, 어긋나 있으면
종료 코드 1 을 낸다. Pages 워크플로는 배포 전에 항상 이 스크립트를 돌린다.

### 변환기가 아는 문법

원본 두 문서가 실제로 쓰는 것만 처리한다. 그 밖의 문법을 만나면 조용히 뭉개지
않고 **오류를 내고 멈춘다.** 법적 문서라 애매하게 넘어가는 것보다 실패가 낫다.

`#` `##` `###` · 문단 · 표(들여쓴 중첩 표 포함) · 번호 목록 · 글머리표(중첩 포함) ·
`>` 인용 · 코드 펜스 · `---` · `**굵게**` · `` `코드` `` · `[링크](url)`

문서 구조에 관한 규칙은 셋뿐이다.

- H1 바로 뒤의 `**...**` 줄들은 개정일·적용 버전으로 보고 `.doc-meta` 로 묶는다.
- `## 요약` 절만 `.notice-box` 로 감싼다. 맨 앞 요약은 본문보다 먼저 읽히는 편이
  낫기 때문이며, 이것이 유일한 예외다.
- 머리말과 꼬리말은 `404.html` 에서 그대로 떼어 온다. 내비게이션을 고치면 이 두
  페이지도 자동으로 따라온다.

### 주의할 점

- 방침 제11조의 제3자 사업자 목록은 원문대로 링크 없이 이름만 둔다. 각 사업자
  정책 URL 은 바뀌는 일이 잦아, 확인 없이 링크를 걸면 오히려 신뢰를 해친다.
- 방침 제5조의 외부 통신 범위는 홈 프라이버시 밴드와 Q&A 답변에도 반영되어 있다.
  이 둘은 원본과 연결되어 있지 않으므로 방침을 고치면 세 곳을 함께 볼 것.
- 개정할 때는 본문뿐 아니라 `**최종 개정일**` 과 부칙 날짜도 함께 고칠 것.
