...
17. [릴리즈 발행과 소개 웹사이트](#17-릴리즈-발행과-소개-웹사이트)
...
├── docs/                            ← 법적 고지 문서 원본 (§17)
│   ├── PRIVACY.md                   ← 개인정보 처리방침. 사이트 HTML 은 여기서 생성된다
│   └── TERMS.md                     ← 이용약관. 위와 같다
├── website/                         ← 소개 웹사이트 (GitHub Pages, §17)
│   ├── README.md                    ← 사이트 구조·생성 규칙 상세
│   ├── *.html                       ← 9개 페이지. privacy/terms 와 updates 일부는 생성물
│   ├── assets/                      ← 스타일·스크립트·폰트·스크린샷
│   └── scripts/
│       ├── sync-release.mjs         ← 버전·크기·SHA-256 주입
│       ├── build-updates.mjs        ← 릴리즈 본문 → 변경 이력
│       ├── build-legal.mjs          ← docs/*.md → 방침·약관
│       └── md-inline.mjs            ← 두 빌더 공용 인라인 마크다운 변환기
├── .eslintrc.json
├── .npmrc                           ← shamefully-hoist=true (ARM64 rollup 호환)
└── .github/workflows/
    ├── ci.yml                       ← 앱 검사 (website/, docs/, README.md 는 제외)
    └── pages.yml                    ← 사이트 생성 + Pages 배포 (§17)
```
...
소개 웹사이트(`website/`)는 <https://gsmtc01.github.io/winguard/> 에 배포된다.
`.github/workflows/pages.yml` 이 배포 때마다 세 스크립트를 돌려 사이트를 다시
만들기 때문에, **사람이 손댈 곳과 손대면 안 되는 곳이 나뉜다.**

| 내용 | 원본 | 사이트에 반영되는 경로 |
|------|------|------------------------|
| 버전·배포일·파일 크기·SHA-256 | GitHub 릴리즈 자산 | `sync-release.mjs` 가 `data-rel` 표시 지점에 주입 |
| 변경 이력 | **GitHub 릴리즈 본문** | `build-updates.mjs` 가 `updates.html` 재생성 |
| 개인정보 처리방침·이용약관 | `docs/PRIVACY.md`, `docs/TERMS.md` | `build-legal.mjs` 가 HTML 재생성 |
| 공지사항 | `website/assets/notices.js` | 브라우저에서 직접 읽음 (생성 없음) |

**생성물을 손으로 고치지 않는다.** `privacy.html`, `terms.html`, 그리고
`updates.html` 의 `<!-- build-updates:start --> ~ <!-- build-updates:end -->`
구간은 다음 배포에서 원본 기준으로 덮어써진다.

### 17.1 릴리즈 노트 작성 형식 (중요)

변경 이력 페이지는 **GitHub 릴리즈 본문에서 자동 생성**된다. 아래 제목이 붙은
절만 사이트로 옮겨지고 나머지는 버려지므로, 형식을 지키지 않으면 사이트에
아무것도 나오지 않는다.

| 릴리즈 본문 | 사이트 |
|-------------|--------|
| 첫 `##` 앞 문단 | 항목 도입 문구 (**첫 문단만**) |
...
다운로드 표, 파일 무결성 확인, AI 모델 안내처럼 사이트의 다른 페이지에 이미
있는 내용은 릴리즈 본문에 그대로 두면 된다. 사이트로 넘어가지 않는다.

#### 템플릿

```markdown
(한 문단으로 이번 릴리즈를 요약한다. 사이트의 도입 문구가 된다.)

## 다운로드

(표·설치 안내. 사이트에는 나오지 않는다.)
...
(해시 대조 방법. 사이트에는 나오지 않는다.)

## 알아두실 점

(경고나 주의사항. 사이트에서 경고 상자로 나온다.)
```

#### 지켜야 할 규칙

1. **목록은 `-` 로 시작하는 줄만 항목이 된다.** 표나 코드 블록을 위 네 절에
   넣으면 버려진다.
2. **em dash(`—`)를 쓰지 않는다.** 사이트 문장 부호 규칙에 어긋나 생성기가
   콜론이나 하이픈으로 바꾸고 경고를 남긴다. 처음부터 콜론으로 쓴다.
   `23개 상시 보안 점검 — DNS 하이재킹` (X) → `23개 상시 보안 점검: DNS 하이재킹` (O)
3. **"위 해시", "아래 표" 같은 문맥 참조를 위 네 절에 넣지 않는다.** 릴리즈
   페이지에서는 바로 위에 그 내용이 있지만 사이트에는 그 절이 오지 않아,
   가리키는 대상이 사라진 문장만 남는다. 필요하면 "다운로드 페이지의 해시로"
   처럼 스스로 완결된 문장으로 쓴다.
4. **버전·파일 크기·SHA-256 을 사이트에 맞춰 적을 필요가 없다.**
   `sync-release.mjs` 가 릴리즈 자산에서 읽어 넣는다. 손으로 적은 값은 어차피
   덮어써지고, 어긋나면 사용자가 무결성 검증에 실패해 정품을 변조된 파일로
   오인한다.
5. **초안(draft)과 사전 릴리즈(prerelease)는 사이트에 실리지 않는다.**
   다운로드 링크가 `releases/latest` 를 가리키는데 GitHub 도 그 대상에서 사전
   릴리즈를 빼므로, 받을 수 없는 버전을 이력에 올리지 않는다.

아는 절이 하나도 없으면 도입 문단과 원문 링크만 남고 워크플로 로그에 경고가
찍힌다. 배포 자체는 계속 진행한다 — 해시 정정이 릴리즈 노트 형식 때문에 막히면
옛 해시가 사이트에 남게 되어 더 위험하기 때문이다.

### 17.2 릴리즈 발행 절차

1. `./build.bat` 으로 x64·ARM64 실행 파일을 만든다 (§14).
2. GitHub 릴리즈를 만들고 실행 파일 2개와 각각의 `.sha256` 파일을 올린다.
   `.sha256` 이 없으면 `sync-release.mjs` 가 실패한다.
3. 릴리즈 본문을 §17.1 형식으로 쓴다.
4. publish 하면 `pages.yml` 이 자동으로 돌아 사이트를 갱신하고 배포한다.

이미 발행한 릴리즈의 본문을 나중에 고쳤다면, Actions 에서 "Pages 배포" 를 수동
실행해야 반영된다(릴리즈 수정은 워크플로를 깨우지 않는다).

### 17.3 앱과 어긋나면 안 되는 사실

사이트가 앱 동작을 설명하는 곳이 여럿이다. 아래를 코드에서 바꾸면 사이트와
법적 고지 문서도 함께 고쳐야 한다.

| 코드 | 사이트에 적힌 곳 |
|------|------------------|
| `app.manifest` 의 `requireAdministrator` | 시스템 요구사항, 설치 가이드, Q&A, 약관 제6조 |
| 코드 서명 적용 여부 | 다운로드 페이지, Q&A, 공지, 약관 제6조, 방침 제8조 |
| AI 모델 크기·저장 위치 | 다운로드 페이지, Q&A, 방침 제5.2조 |
| 외부 통신 대상 호스트 | 방침 제5조, 홈 프라이버시 밴드, Q&A |
| `PRUNE_THRESHOLD` (격리 50건) | 방침 제4조, 약관 제8조, Q&A |
| `MAX_ENTRIES` (VT 이력 20건) | 방침 제4.1조 |
| `engine/scorer.rs` 의 감점 폭 | Q&A "보안 점수는 어떻게 계산되나요" |

상세한 사이트 구조와 생성 규칙은 `website/README.md` 에 있다.
